...
# Put our vendored wheels ahead of any /opt/python layer
VENDOR = os.path.join(os.path.dirname(__file__), "vendor")
if VENDOR not in sys.path:
    sys.path.insert(0, VENDOR)

# Now imports will come from ./vendor (our pinned wheels)
...
def lambda_handler(event, context):
    # small diagnostics so we can verify runtime is correct
    log.info("[versions] weasyprint=%s pydyf=%s", getattr(weasyprint, "__version__", "?"),
