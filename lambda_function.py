...
PDF_ENABLED         = True
PDF_FORMAT          = "A4"
PDF_MARGIN_MM       = {"top":"10mm","right":"10mm","bottom":"10mm","left":"10mm"}
PDF_WAIT_NETWORK_MS = 20000   # wait for network idle after setContent
PDF_CHARTS_WAIT_MS  = 10000   # extra wait for Google Charts to draw
# =========================
...
# ------------------------ HTML → PDF (Playwright) -------------------
def html_to_pdf_bytes(html: str, base_url: Optional[str] = None) -> Optional[bytes]:
    """
    Render HTML to PDF using Playwright/Chromium.
    Returns PDF bytes, or None on failure (caller can fallback).
    """
    if not PDF_ENABLED:
        return None
    try:
        ...
        log(f"[warn] Playwright not available: {type(e).__name__}: {e}")
        return None
    ...
    try:
        with sync_playwright() as p:
            ...
            # Allow some time for heavy pages to settle and charts to draw
            page.set_default_timeout(PDF_WAIT_NETWORK_MS)

            # If your Lambda runs in a private subnet without NAT, external scripts will fail.
            # In that case, either attach NAT or self-host the JS. We still proceed to print.
            if base_url:
                page.set_content(html, wait_until="networkidle", base_url=base_url)
            else:
                page.set_content(html, wait_until="networkidle")

            # Try to wait for Google Charts (SVGs) to finish painting; ignore timeout.
            try:
                page.wait_for_function(
                    "(() => window.google && google.charts && document.querySelectorAll('svg').length >= 3)()",
                    timeout=PDF_CHARTS_WAIT_MS
                )
            except Exception:
                log("[info] Charts wait timed out; proceeding to print")
        ...
        log(f"[warn] PDF generation failed: {type(e).__name__}: {e}")
        return None
...
def handler(event, context):
    ...
    try:
        ...
        s3.put_object(
            ...
            Key=f"{REPORTS_PREFIX}/{y}/uptime-year-summary.csv",
            Body=io.StringIO().getvalue().encode("utf-8"),  # placeholder to satisfy lints (we write real CSV below)
            ContentType="text/csv; charset=utf-8"
        )
        # Proper write:
        out = io.StringIO()
        w = csv.DictWriter(out, fieldnames=["month","availability_pct","avg_response_sec"])
        w.writeheader()
        for r in year_csv: w.writerow(r)
        s3.put_object(Bucket=REPORTS_BUCKET, Key=f"{REPORTS_PREFIX}/{y}/uptime-year-summary.csv",
    ...
    # HTML → PDF
    pdf_key = f"{base}uptime-report.pdf"
    pdf_uri = None
    if PDF_ENABLED:
        ...
        if pdf_bytes:
            ...
            pdf_uri = f"s3://{REPORTS_BUCKET}/{pdf_key}"
            log(f"[info] PDF saved to {pdf_uri}")
        else:
            log("[warn] PDF generation disabled or failed; HTML only")

    return {
        "status":"ok",
        "artifacts": {"bucket": ART_BUCKET, "prefix": ART_PREFIX},
        "reports": {"bucket": REPORTS_BUCKET, "prefix": REPORTS_PREFIX},
        "result_html": {"bucket": REPORTS_BUCKET, "key": html_key},
        "result_pdf": {"bucket": REPORTS_BUCKET, "key": pdf_key} if pdf_uri else None,
